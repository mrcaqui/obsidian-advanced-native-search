...
### 今後のタスク
- Regex/Simple/Fuzzy それぞれで代表的なケースの自動テストを追加（line AND・glob・frontmatter 配列・タグ正規化など）。  
- 大規模 Vault/巨大ノートでのベンチマークとプロファイリング（行スキャンの打ち切り条件や per-file 抜粋上限の最適化）。  
- ハイライト・検索ユーティリティのモジュール分割（将来的な再利用/単体テスト容易化）。  
- README/ヘルプに仕様（Global Query と filters の関係、regex フラグ挙動、sort/limit の動作）を追記。
