...
### 今後のタスク
- ネイティブの「N results」に近づける出現回数ベースの集計オプションを検討・実装（同一行に複数回出現する語を合算）。  
- `file:` の一致対象を拡張子除去のベース名に切り替える設定を追加（必要に応じて選択可能に）。  
- `.md` 以外の拡張子対応や検索範囲設定のUI拡充。  
- パフォーマンス計測と最適化（大規模Vaultでの応答性改善、必要に応じて検索の非同期化）。  
- 単体テストの整備（line AND論理、propertyフィルタ、tag抽出の妥当性検証）。
