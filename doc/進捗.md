...
- Global Query のヒット語句ハイライトを結果抜粋にも反映（simple/fuzzy/regex 各モードに対応）。
- 大量結果時のパフォーマンス最適化（仮想リスト／バーチャルスクロール導入）。
- 設定項目の拡充（分割方向［右/下］や初期比率の指定）。
- クイックフィルタ（タグ／パスなど）のオンオフ、ソート切替 UI の追加。
- 空結果時の状態表示やエラー表示の強化（ユーザーフィードバック改善）。
